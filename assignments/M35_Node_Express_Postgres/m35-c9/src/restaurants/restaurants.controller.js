...
async function listAverageRatingByOwner(req, res, next) {
  const ratingByOwner = await service.listAverageRatingByOwner();
// console.log("ratingByOwner: ", ratingByOwner);
  const numChange = ratingByOwner.map((rating)=>{
    return {owner_name: rating.owner_name, avg: Number(rating.avg)}
  })
  res.json({ data: numChange});
}
