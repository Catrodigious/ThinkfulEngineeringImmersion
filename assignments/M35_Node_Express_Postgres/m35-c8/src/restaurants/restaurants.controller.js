...
async function averageRating(req, res, next) {
  const avgRating = await service.averageRating();
  console.log("avgRating: ", avgRating);
  const {avg} = avgRating;
  const numAvg = Number(avg).toPrecision(4);
  res.json({ data: {average_rating: Number(numAvg) }});
}
...
async function count(req, res, next) {
  // your solution here
  const restCount = await service.count();
  res.json({ data: restCount });
}
...
async function readHighestRating(req, res, next) {
  ...
  const returnObj = {max_rating: highestRating[0].max};
  res.json({ data: returnObj });
}
