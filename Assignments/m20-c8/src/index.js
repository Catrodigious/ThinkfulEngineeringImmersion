...
function filterHandler(evt) {
  ...
  if (city === "0"){
    ...
    const contactsByCity = filterByCity(city);

    const contactElement = document.querySelector("#contacts");
    contactElement.innerHTML = "";

    contactsByCity.map((contact)=>{
      let newElement = renderContact(contact);
      contactElement.innerHTML += newElement;
    })
  };
...
function loadCities(allContacts) {
  const citySelection = document.querySelector('#filterOptions');

  if (allContacts && allContacts.length > 0){
    const cities = allContacts.map((contact)=>contact.address.city);
    const uniqueCities = new Set(cities);
    const citiesArr = Array.from(uniqueCities);
    ...
    const options = citySelection.querySelectorAll("option");
...
function main() {
  ...
  const toDelete = Array.from(document.querySelectorAll(".deleteBtn"));
